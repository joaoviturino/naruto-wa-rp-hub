...
const npcPayload = z.object({
  ...
  hp_max: z.number().int().min(1).max(1_000_000_000),
  xp: z.number().int().min(0).max(1_000_000_000),
  energy_max: z.number().int().min(1).max(1_000_000_000),
  reward_xp: z.number().int().min(0).max(1_000_000_000).optional(),
  reward_ryo: z.number().int().min(0).max(1_000_000_000).optional(),
  avg_damage: z.number().int().min(0).max(1_000_000_000).optional(),
  ...
  kind: z.enum(["aggressive","shop","reward","learning","object","dialogue","buyer","employer"]).optional(),
  offered_job_id: z.string().uuid().nullish(),
