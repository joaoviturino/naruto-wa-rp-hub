  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: char, error } = await context.supabase
      ...
      .insert({
        ...
        body_sheet_url: '/__l5e/assets-v1/0e0be9a3-2dd3-4725-b0c0-b3b8c7aac008/shinobi-base-sheet.png',
        body_sheet_cols: 6,
        body_sheet_rows: 7,
        body_sheet_states: {
          idle:  { row: 0, frames: 6, fps: 4,  loop: true },
          run:   { row: 1, frames: 6, fps: 10, loop: true },
          punch: { row: 2, frames: 6, fps: 12, loop: false },
          kick:  { row: 3, frames: 6, fps: 12, loop: false },
          hurt:  { row: 4, frames: 6, fps: 10, loop: false },
          cast:  { row: 5, frames: 6, fps: 8,  loop: true },
